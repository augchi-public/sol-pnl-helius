...
export { computeSolBalanceOverTimeV3 };
...
async function computeSolBalanceOverTimeV3(
  ...
  // Large wallet (1000+): sig-sweep → targeted full fetch
  debugLog("algo", "large wallet — sig-sweep strategy", { quickCount });
  ...
  if (capped) {
    ...
    // Split the full range into concurrency-sized chunks and sweep all sigs.
    // We already have 1000 sigs from Round 0 (bounds.sigSlots for desc direction).
    // Now sweep the full range in parallel to get ALL sig slots.
    ...
    // Deduplicate consecutive identical slots (keep count accurate)
    const uniqueSlots = [...new Set(allSigSlots)];
    ...
    // ── Phase 2: Targeted full fetch with perfectly-sized ranges ──
    // Group sigs into chunks of ~90 so each full fetch is guaranteed
    // to return ≤100 txs (zero pagination needed).
    const TARGET_PER_CHUNK = 97;
    const fetchRanges = buildBalancedRanges(allSigSlots, fullRange, TARGET_PER_CHUNK);
    debugLog("algo", "targeted full fetch", {
